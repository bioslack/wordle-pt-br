...
var _wordle = require("./js/wordle");
const board = new (0, _wordle.Board)("ptbr");
board.render();
const letters = "abcdefghijklmnopqrstuvwxyz";
document.addEventListener("keydown", function(event) {
    if (letters.includes(event.key) && !board.game.isGameOver()) board.editGuess(event.key);
    if (event.key === "Backspace") board.deleteChar();
    if (event.key === "Enter") board.reveal();
});
document.addEventListener("click", function(event) {
    if (event.target.classList.contains("button")) board.newGame();
});

},{"./js/wordle":"g4teI"}],"g4teI":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "Wordle", ()=>Wordle);
parcelHelpers.export(exports, "Board", ()=>Board);
var _wordlistBr = require("./wordlist-br");
var _wordlistBrDefault = parcelHelpers.interopDefault(_wordlistBr);
var _lodash = require("lodash");
const wordlists = {
    ptbr: (0, _wordlistBrDefault.default)
};
class GameElement {
    render() {}
}
...
const removeSpecialCharacters = function(word) {
    return word.replace(/[ÀÁÂÃÄ]/, "A").replace(/[àáâãä]/, "a").replace(/[ÈÉÊË]/, "E").replace(/[èéêë]/, "e").replace(/[ÌÍ]/, "I").replace(/[ìí]/, "i").replace(/[ÒÓÔÕÖ]/, "O").replace(/[òóôõö]/, "o").replace(/[ÙÚÜ]/, "U").replace(/[ùúü]/, "u").replace(/[Ç]/, "C").replace(/[ç]/, "c").toLowerCase();
};
class SidePanel extends GameElement {
    ...
    render() {
        ...
        const results = this.pastResults.map((r)=>r.counter).reduce((sum, current)=>sum += current, 0);
        this.pastResults.forEach((element, i)=>{
            const line = document.createElement("div");
            line.classList.add("side-panel__results-line");
            const indexContainer = document.createElement("div");
            indexContainer.style = "width: 20px; text-align: center; margin-right: 10px;";
            ...
            gaugeContainer.style.transform = `scaleX(${this.pastResults[i].counter / (results || 0.00000000001) * 100}%)`;
...
class Wordle {
    ...
    isAValidGuess(guess) {
        return this.wordlist.map((w)=>removeSpecialCharacters(w)).includes(guess.toLowerCase());
    }
    isGameOver() {
        return this.hasWon() || this.matrix.length === 6;
    }
    hasWon() {
        return this.matrix.length > 0 && removeSpecialCharacters(this.answer) === this.matrix[this.matrix.length - 1].map((l)=>l.letter).join("");
    }
    guess(word) {
        ...
        const guessInWordlist = this.wordlist.find((w)=>removeSpecialCharacters(w) === word);
...
class Line extends GameElement {
    ...
    reveal(line) {
        ...
        this.squares.forEach(function(square, i) {
            const revealFunction = ()=>setTimeout(()=>{
    ...
    render() {
        ...
        for(let i = 0; i < 5; i++){
            ...
            } else if (i < this.guessWord.length) back.innerHTML = this.guessWord[i];
...
class Board extends GameElement {
    ...
    reveal() {
        ...
        if (!this.wordlist.map((w)=>removeSpecialCharacters(w)).includes(this.currentGuess)) {
    ...
    render() {
        ...
        this.lines = [];
        for(let i = 0; i < 6; i++){
            const line = i < this.game.matrix.length ? new Line(this.game.matrix[i]) : new Line([]);
...
},{"./wordlist-br":"1EFBG","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","lodash":"3qBDj"}],"1EFBG":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
exports.default = [
    "aar\xe3o",
...
},{}]},["8TtF2","gLLPy"], "gLLPy", "parcelRequire106d")
